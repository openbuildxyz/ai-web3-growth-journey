...
export class TokenMinter {
  ...
  constructor() {
    // 使用多个RPC端点，优先使用更稳定的端点
    const rpcEndpoints = [
      'https://api.devnet.solana.com',
      'https://devnet.helius-rpc.com/?api-key=82c2f8e7-7d6c-4b8b-8b8b-8b8b8b8b8b8b',
  ...
  async mintTokens(toAddress: string, amount: number): Promise<string> {
    ...
    try {
      console.log(`Minting ${amount} ${TOKEN_CONFIG.symbol} to ${toAddress}...`);
      ...
      console.log(`Successfully minted ${amount} ${TOKEN_CONFIG.symbol} to ${toAddress}`);
